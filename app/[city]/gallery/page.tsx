...
export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  ...
  return city ? { title: `${city.name} photos | Arc Trips` } : { title: "Arc Trips" };
}

export default async function GalleryIndexPage({ params }: { params: Promise<{ city: string }> }) {
  const { city: citySlug } = await params;
  ...
  return (
    <>
      ...
      />
      <CategoryIndex city={city} categories={categories} mode="gallery" />
    </>
  );
}
