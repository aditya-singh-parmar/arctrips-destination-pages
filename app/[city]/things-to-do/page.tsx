...
export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  ...
  return city ? { title: `Things to do in ${city.name} | Arc Trips` } : { title: "Arc Trips" };
}

export default async function ThingsToDoPage({ params }: { params: Promise<{ city: string }> }) {
  const { city: citySlug } = await params;
  ...
  return (
    <>
      ...
      />
      <CategoryIndex city={city} categories={categories} mode="things-to-do" />
    </>
  );
}
